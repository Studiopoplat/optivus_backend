...
# -----------------------------
async def list_transactions(user, page: int, page_size: int, db: AsyncSession = Depends(get_db)) -> List[TransactionResponse]:
...
# -----------------------------
async def get_transaction(user, transaction_id: str, db: AsyncSession = Depends(get_db)) -> TransactionResponse:
...
# -----------------------------
async def create_deposit(user, amount: str, currency: str = "usd", db: AsyncSession = Depends(get_db)):
...
# -----------------------------
async def log_withdrawal(user_id: str, amount: str, currency: str = "usd", db: AsyncSession = Depends(get_db)):
    tid = str(uuid4())
...
# LOG REFERRAL BONUS
# -----------------------------
async def log_referral_bonus(user_id: str, referee_id: str, amount: str, db: AsyncSession = Depends(get_db)):
    tid = str(uuid4())
